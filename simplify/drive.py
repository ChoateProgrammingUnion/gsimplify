...
class Drive:
    def __init__(self, drive_id: str, service: Resource):
        ...
        self.items = self.contains()
        
    def contains(self):
        drive_obj = self.service.files().list(
            corpora='teamDrive',
        ...
        for each_obj in drive_obj:
            if each_obj.get('mimeType') == "application/vnd.google-apps.document":
                container.append(Doc(kind=each_obj.get('kind'), id=each_obj.get('id'), name=each_obj.get('name'), mime_type = each_obj.get('mimeType'), drive_id = each_obj.get('driveId')))
    ...
    def docs(self):
        return [x for x in self.items if isinstance(x, Doc)]

    def folders(self):
        return [x for x in self.items if isinstance(x, Folder)]
